...
namespace Common
{
    ...
    {
        public static List<byte[]> Slice(byte[] b)
        {
            ...
            {
                hash = GetSubsequentHash(b[j - 1], b[j + windowSize - 1], hash, Q, D, power);
        ...
        public static ulong GetSubsequentHash(byte removeByte, byte addByte, ulong hash, ulong Q, ulong D, ulong power)
        {
            // add next element to window
            hash = (hash + Q - power * removeByte % Q) % Q;
